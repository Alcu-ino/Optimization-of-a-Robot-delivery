ENTRATE = [[41.108763, 16.880046]; [41.109204, 16.879889];[41.108310, 16.879877]]
%[[41.108763, 16.880046]; [41.109204, 16.879889];[41.108310, 16.879877];[41.108090, 16.880510];[41.108743, 16.878600];[41.109187, 16.878415];[41.109404, 16.879188];[41.109759, 16.878875];[41.108498, 16.880554];
...
for i = 1:nEnt
    for j = i+1:nEnt
        ...
        tp = max(1, ceil(60 * Lkm ./ velocita / 15));             % minuti
...
%% ---------- ESTRAZIONE INCROCI TRA I CLIENTI ----------

nodiIncrocioUnici = unique(nodiIncrocioUsati);
% Calcola il grado di ogni nodo nel grafo
gradiNodi = degree(G);

% Filtriamo 'nodiIncrocioUnici' tenendo solo quelli il cui grado nel grafo è >= 3
% (puoi usare >= 3 per intersezioni vere e proprie o >= 2 se consideri anche i cambi di direzione)
...
% Aggiorna le coordinate corrispondenti
latIncroci = nodeLat(nodiIncrocioUnici);
lonIncroci = nodeLon(nodiIncrocioUnici);

fprintf("Trovati %d incroci stradali effettivi (grado >= %d) utilizzati nei percorsi.\n", ...
    numel(nodiIncrocioUnici), gradoMinimoIncrocio);

fprintf("Trovati %d incroci stradali unici utilizzati nei percorsi tra le entrate.\n", numel(nodiIncrocioUnici));
%%
filename = "/home/vito/Scrivania/lib/export.osm";
...
hold on
geoplot(shpIncroci, "o", "MarkerFaceColor", [0.9 0.1 0.1], ...
